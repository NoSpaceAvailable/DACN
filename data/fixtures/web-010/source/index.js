...
router.get('/', async ctx => {
  ...
  <!--
  POST /oauth2/token
  POST /oauth2/authorize form-data
  --!>
  `;
});
...
router.post('/login', async ctx => {
  ...
  ctx.body = `<p>Successfully logged in!</p>`;
});
...
async function authorizationCode(data) {
  return new Promise((resolve, reject) => {
    jwt.sign({ ...data }, process.env.JWT_SECRET || 'keyboard_cat', { expiresIn: '10m' },
      (err, token) => { if (err) return reject(err); return resolve(token); });
  });
}
...
router.post('/oauth2/authorize', checkLogin, async ctx => {
  ...
  if (response_type == 'code') {
    const code = await authorizationCode({
      client_id, redirect_uri, type: ctx.session.type,
    });
...
router.post('/oauth2/token', async ctx => {
  ...
  if (grant_type == 'authorization_code') {
    ...
    ctx.body = { token_type: 'Bearer', token };
    return;
  }
...
async function checkAuthentication(ctx, next) {
  ...
  const auth = await verifyToken(token).catch(err => {
    return ctx.throw(401, "Your token could not be verified.");
  });
...
app.use(logger());
app.use(bodyParser());
app.use(session({ key: 'CSAW-CTF-2018-QUALS-SSO' }, app));
