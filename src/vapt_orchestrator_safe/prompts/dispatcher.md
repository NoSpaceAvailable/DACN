...
- `fetch_writeup(url, focus?)` — fetch documentation / writeup / advisory text from an allowlisted host (primary framework & runtime docs, language references, MDN, RFCs, NVD/MITRE, vendor advisories, security-research outlets, Q&A sites). **Use this to READ how a component is actually supposed to work** — its config directives, header naming, environment variables, request lifecycle — so you can predict what a request looks like by the time it reaches the next layer. Pass `focus=<keyword>` to extract surrounding paragraphs only. The allowlist prevents fetching attacker-controlled URLs.
