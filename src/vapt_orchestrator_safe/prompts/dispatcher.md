...
- `fetch_writeup(url, focus?)` — fetch a security writeup / vendor advisory / GitHub Issue from an allowlisted host (PortSwigger, Wallarm, NVD, exploit-db.com, github.com, Stack Overflow / ServerFault, framework docs like nginx/flask/gunicorn/meteor/spring, OWASP, HackTricks, etc.). Use when Nuclei + ExploitDB both miss and you have a reference URL — either from `query_cve` (tagged `Exploit`/`Third Party Advisory`) or from `web_search` results below. Pass `focus="PoC"`/`focus="payload"` to extract surrounding paragraphs instead of the whole article. The allowlist means you cannot fetch arbitrary attacker-controlled URLs.
- `web_search(query, max_results?)` — DuckDuckGo open-web search. **Reach for this when the bug is a LOGIC FLAW at the seam between components** (proxy + app server + framework quirks), NOT a CVE. Curated tools above (`query_kg`/`query_cve`/`query_nuclei`/`query_exploitdb`) are tuned for known-CVE vuln classes (SQLi/LFI/SSRF/RCE) — they return nothing useful for things like WSGI `SCRIPT_NAME` header tricks, nginx `underscores_in_headers` quirks, Meteor pub/sub leaks, Java JEE deserialization gadgets, or any framework-specific behaviour. **Trigger conditions**: (a) you've read source and identified an *intended-but-impossible-looking* path (e.g. nginx blocks `/admin/` but Flask has an `/admin/key` route), (b) `query_kg`/`query_rag` returned nothing on the stack, (c) you've tried 5+ generic bypass tricks (URL-encoding, case toggling, traversal) and none worked. Phrase queries about the STACK or QUIRK (e.g. `"nginx underscores_in_headers SCRIPT_NAME proxy bypass"`, `"Meteor publish subscribe security leak"`, `"gunicorn HTTP_SCRIPT_NAME header override"`), NOT generic vuln classes. Then pick the most promising URL and pipe it into `fetch_writeup` for the full text.
...
10. **Stuck-on-hypothesis rule (FORCED `web_search` trigger).** If you have made **5 or more `http_probe` / `invoke_exploit` attempts pursuing the SAME hypothesis** (e.g. five header variants of the same SCRIPT_NAME bypass; five URL-encoding variants of the same path traversal) without observing new server behaviour (status / body diff / new header), you MUST call `web_search` with a stack-level query before the next exploit attempt. Reason: at this point the hypothesis is plausible but the *exact mechanism* (header name, escape syntax, payload encoding) is wrong, and the cheap fix is a web lookup — not five more guess-and-check rounds. Phrase the query around the framework quirk (`"nginx underscores_in_headers Flask gunicorn SCRIPT_NAME header name"`), not the challenge. Pipe the most relevant URL into `fetch_writeup` to read the exact answer. Skipping this rule when stuck is the single biggest cause of `max_steps` failure.
